	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, r := range results {
				...
				var minioVal string
				if strings.Contains(r.MinIOState, "running") {
					minioVal = successText.Render("running")
				} else if r.MinIOState != "" && r.MinIOState != "inactive" {
					minioVal = warnText.Render(r.MinIOState)
				} else {
					minioVal = warnText.Render("inactive")
				}
